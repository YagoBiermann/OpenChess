...
namespace OpenChess.Tests
{
    ...
    {
        public static MatchInfo RestoreMatch(string fen, string mId, string p1Id, string p2Id, int mtime = 5, string mstatus = "InProgress", string? winner = null, string checkState = "NotInCheck")
        {
            string matchId = mId;
            ...
            MatchInfo matchInfo = new(matchId, players, fen, pgnStack, status, time, checkState, winner);

            return matchInfo;
        }

        public static MatchInfo RestoreMatch(string fen, string checkState = "NotInCheck")
        {
            ...
            MatchInfo matchInfo = new(matchId, players, fen, pgnStack, status, time, checkState);

            return matchInfo;
        }

        public static Match RestoreAndPlay(string fen, string origin, string destination, string? promoting = null, string checkState = "NotInCheck")
        {
            string matchId = Guid.NewGuid().ToString();
            PlayerInfo player1 = new(Guid.NewGuid().ToString(), 'w', matchId);
            PlayerInfo player2 = new(Guid.NewGuid().ToString(), 'b', matchId);
            List<PlayerInfo> players = new() { player1, player2 };
            MatchInfo matchInfo = new(matchId, players, fen, new(), MatchStatus.InProgress.ToString(), 5, checkState);
